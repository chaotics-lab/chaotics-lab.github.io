  ...
  return (
    <div className="relative w-full text-white flex flex-col overflow-hidden min-h-screen bg-starfield">
      ...
      <style>{`
        ...
        .markdown-scroll::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.3); border-radius: 9999px; }
          ...
          <div className="relative w-full h-full flex items-center justify-center p-4 md:p-8" onClick={(e) => e.stopPropagation()}>
            <div className="relative w-full h-full max-w-7xl max-h-full flex items-center justify-center">
              ...
              {frameCount > 1 && (
                <>
                  {currentFrame > 0 && (
                    <button onClick={(e) => { e.stopPropagation(); handlePrevFrame(); }} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 md:p-4 rounded-full transition-all z-[105]">
                      <ArrowLeft size={24} />
                    </button>
                  )}
                  {currentFrame < frameCount - 1 && (
                    <button onClick={(e) => { e.stopPropagation(); handleNextFrame(); }} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 md:p-4 rounded-full transition-all z-[105]">
            ...
            {frameCount > 1 && (
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 z-[105]">
                {Array.from({ length: frameCount }, (_, i) => (
                  <button
                    key={i}
                    onClick={(e) => { e.stopPropagation(); setCurrentFrame(i); setIsPaused(true); setTimeout(() => setIsPaused(false), 5000); }}
      ...
      <div className={`relative z-10 container mx-auto py-4 md:py-8 px-4 flex flex-col gap-4 ${isMobile ? '' : 'h-screen overflow-hidden'}`}>
        ...
        <Link to="/" className="shrink-0">
          <button className="group relative flex items-center gap-2 px-4 py-2 text-sm font-ui overflow-visible rounded-xl transition-all duration-300 hover:-translate-y-0.5">
            ...
            <span className="relative text-space-muted group-hover:text-white transition-colors duration-300">Back</span>
          </button>
        </Link>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center gap-4 mb-3 flex-wrap">
                ...
                {date && <div className="text-sm text-white/70 font-ui font-medium tracking-wide shrink-0">{date.toLocaleString("default", { month: "long" })} {date.getFullYear()}</div>}
                {project.type && <Badge variant="outline" className="font-ui text-xs md:text-sm bg-white/10 border-white/20 shrink-0">{project.type}</Badge>}
              </div>
